...
def make_linear_layer(
    ...
    """
    A helper function to build linear layers wrapped with spectral norm.
    """
...
def masked_mean_pool(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """
    Mean-pools the sequence dimension of x using a binary mask, corresponding to non-padding tokens.
